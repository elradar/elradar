...
categories: [uncranalavez, janitor, bibliotecas]
tags: [limpieza, transformacion, ETL, data-wrangling]
...
Lo que nos va a interesar en este caso es hacer un análisis de las variables género y apoyo espiritual. Ahora, ¿no sería más útil tener en vez de categorías A y B más información sobre las categorías? Consultando el libro de código de esta base (si, esa prolijidad por favor para documentar todes) sé que A significa hombre en género (ninguna sorpresa por aquí) y B mujer (nosotres no la hicimos les juro). Además, que en apoyo espiritual A significa que no lo solicitó y B que sí lo solicitó y lo tuvo (existe otra categoría que lo solicitó y no lo tuvo pero no nos tocaron casos). Vamos a usar la función recode() del tidyverse para emprolijarla.
Por último, vimos que había muchas celdas vacías que no eran tomadas como casos perdidos aunque conceptualmente lo son. Vamos a ayudarnos con mutate_all() y na_if() para resolver eso.
...
Una gran ventaja de janitor es que nos permite además es hacer tablas relativamente lindas de manera sencilla y compatibles con el pipe de magrittr (%>%) que popularizó el tidyverse. Vamos a cruzar la variable género con apoyo espiritual.
